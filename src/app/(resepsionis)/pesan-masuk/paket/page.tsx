...
export default function PaketMasukPage() {
  ...
  const filteredData = data.filter((item) => {
    const matchesSearch = 
      ...
      item.ekspedisi.toLowerCase().includes(search.toLowerCase()) ||
      (item.namaPengirim?.toLowerCase() || "").includes(search.toLowerCase()) ||
      (item.noResi?.toLowerCase() || "").includes(search.toLowerCase());
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Paket Masuk");
    XLSX.writeFile(wb, `Paket_Masuk_${format(new Date(), "yyyy-MM-dd")}.xlsx`);
  };

  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text("Laporan Paket Masuk", 14, 15);
    autoTable(doc, {
      startY: 20,
      head: [['Penerima', 'Ekspedisi', 'Pengirim', 'No. Resi', 'Tanggal', 'Jam']],
    ...
    doc.save(`Paket_Masuk_${format(new Date(), "yyyy-MM-dd")}.pdf`);
  };

...
export default function TeleponTable({ teleponList, loading, onEdit, onDelete }: TeleponTableProps) {
  ...
  const totalPages = rowsPerPage === "all" ? 1 : Math.ceil(teleponList.length / rowsPerPage);
  const indexOfLastRow = currentPage * (rowsPerPage === "all" ? teleponList.length : rowsPerPage);
  const indexOfFirstRow = rowsPerPage === "all" ? 0 : indexOfLastRow - rowsPerPage;
  const currentRows = rowsPerPage === "all" ? teleponList : teleponList.slice(indexOfFirstRow, indexOfLastRow);
